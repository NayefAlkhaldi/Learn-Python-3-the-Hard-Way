...
class Sentence(object):
    def __init__(self, subject, verb, obj, number=None):
...
class Parser(object):
    ...
    def parse_number(self, word_list):
        ...
        raise ParserException("Expected a number next.")

    def parse_sentence(self, word_list):
        subj = self.parse_subject(word_list)
        verb = self.parse_verb(word_list)
        obj = self.parse_object(word_list)
        num = self.parse_number(word_list)
        if not num:
            return Sentence(subj, verb, obj)
        else:
            return Sentence(subj, verb, obj, number=num)